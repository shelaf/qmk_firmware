...
void CLK_dfll_init(void) {
    system_clks.freq_dfll    = FREQ_DFLL_DEFAULT;
    system_clks.freq_gclk[0] = system_clks.freq_dfll;

    // usec_delay_mult = FREQ_DFLL_DEFAULT / (USEC_DELAY_LOOP_CYCLES * 1000000);
...
void CLK_reset_time(void) {
    ...
    while (ptc4->COUNT16.STATUS.bit.SYNCBUSY) {
    }
    ptcc0->CTRLA.bit.ENABLE = 0;
    while (ptcc0->SYNCBUSY.bit.ENABLE) {
    }
    // zero counters
    ...
    ptcc0->COUNT.reg = 0;
    while (ptcc0->SYNCBUSY.bit.COUNT) {
    }
    // start counters
    ptcc0->CTRLA.bit.ENABLE = 1;
    while (ptcc0->SYNCBUSY.bit.ENABLE) {
    }
...
void CLK_delay_us(uint32_t usec) {
    uint32_t usec_delay_mult = FREQ_DFLL_DEFAULT / (USEC_DELAY_LOOP_CYCLES * 1000000);
    asm(".syntax unified\n\t"
        "cmp r0, #0\n\t"
        "beq 2f\n\t"
        "muls r0, %0\n\t"
...
void clk_enable_sercom_apbmask(int sercomn) {
    ...
    switch (sercomn) {
        case 0:
            pmclk->APBCMASK.bit.SERCOM0_ = 1;
            break;
        case 1:
            pmclk->APBCMASK.bit.SERCOM1_ = 1;
            break;
        case 2:
            pmclk->APBCMASK.bit.SERCOM2_ = 1;
            break;
        case 3:
            pmclk->APBCMASK.bit.SERCOM3_ = 1;
            break;
        case 4:
            pmclk->APBCMASK.bit.SERCOM4_ = 1;
            break;
        case 5:
            pmclk->APBCMASK.bit.SERCOM5_ = 1;
            break;
        default:
            break;
    }
...
uint32_t CLK_set_i2c0_freq(uint8_t sercomn, uint32_t freq) {
    ...
    uint32_t clkctrl_id = (GCLK_CLKCTRL_ID_SERCOM0_CORE_Val + sercomn) << GCLK_CLKCTRL_ID_Pos;
    pgclk->CLKCTRL.reg = GCLK_CLKCTRL_CLKEN | GCLK_CLKCTRL_GEN_GCLK0 | clkctrl_id;
