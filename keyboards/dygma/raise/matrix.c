...
extern uint8_t tmp_mtx[5];
extern uint8_t tmp_mtx_right[5];
...
//    uint32_t scans[2]; //PA PB
	...
	for(int i=0; i<5; i++) {
	    mlatest[i+5] = tmp_mtx_right[i];
	}
    }
#endif 

#if 0
    // mlatest[7] |= ((PORT->Group[0].IN.reg & (1 << 23)) ? 0 : 0x80);
    // mlatest[8] |= ((PORT->Group[0].IN.reg & (1 << 23)) ? 0 : 0x04);
    // mlatest[1] |= ((PORT->Group[0].IN.reg & (1 << 23)) ? 0 : 0x02);
